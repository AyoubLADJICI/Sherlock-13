	...
	for (i=0;i<4;i++) {
     ...
     while (1) {    
        ...
        if (fsmServer==0) {
			...
			switch (buffer[0]){
				// Message 'G' : le joueur accuse un coupable
				case 'G':
				int id_coupable;
				sscanf(buffer,"G %d %d", &id, &id_coupable); // recupère l'id du joueur et le personnage qu'il accuse
				if (id_coupable == deck[12]){ // vérifie si la personne suspectée est bien le coupable
					sprintf(reply, "W %d", id);
					broadcastMessage(reply); // envoie le message de victoire à tous les joueurs
				}else {
					sprintf(reply, "W %d", id);
					broadcastMessage(reply); // envoie le message de défaite à tous les joueurs
				}
                ...
                case 'O':
				int id_objet;
				sscanf(buffer, "O %d %d", &id, &id_objet); // recupère l'id du joueur et l'objet qu'il accuse
				printf("id=%d objet=%d\n", id, id_objet);
				for (i=0;i<4;i++) {
					if (i!=id) {
						if (tableCartes[i][id_objet] > 0) {
							sprintf(reply, "V %d %d %d", i, id_objet, 100);
						} else {
							sprintf(reply, "V %d %d %d", i, id_objet, 0);
						}
						broadcastMessage(reply);
					}
				}
				...
				case 'S':
				int joueur_cible, objet;
				sscanf(buffer, "S %d %d %d", &id, &joueur_cible, &objet);
				sprintf(reply, "V %d %d %d", joueur_cible, objet, tableCartes[joueur_cible][objet]);
