...
int main(int argc, char ** argv) {
    ...
    int quit = 0; //permet de quitter SDL
    SDL_Event event; //permet de gerer les evenements tels que clic souris, evenement clavier, etc.
	int mx,my; //mouse x et mouse y coordonnees
	char sendBuffer[256];
	char lname[256];
	int id;
    ...
    //avant de recevoir les noms des joueurs via la recepion du message L du serveur
	//on les initialise a "-"
	strcpy(gNames[0],"-");
	strcpy(gNames[1],"-");
	strcpy(gNames[2],"-");
		...
		if (SDL_PollEvent(&event)) {
        	...
        	switch (event.type) {
				...
				case SDL_MOUSEBUTTONDOWN:
					...
					//printf("mx=%d my=%d\n",mx,my);
					//si le joueur clique sur le bouton connect
					if ((mx<200) && (my<50) && (connectEnabled==1)) {
						sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName);
						//on envoi le message au serveur (on indique l'adresse ip, le port et le nom du joueur)
						sendMessageToServer(gServerIpAddress,gServerPort,sendBuffer);
						connectEnabled=0;
					//si le joueur clique sur un joueur dans la colonne à gauche
					}else if ((mx>=0) && (mx<200) && (my>=90) && (my<330)) {
						joueurSel=(my-90)/60;
						guiltSel=-1;
					//si le joueur clique sur un objet dans la ligne d'en haut
					}else if ((mx>=200) && (mx<680) && (my>=0) && (my<90)) {
						objetSel=(mx-200)/60;
						guiltSel=-1;
					//si le joueur clique sur un personnage à accuser
					}else if ((mx>=100) && (mx<250) && (my>=350) && (my<740)) {
						joueurSel=-1;
						objetSel=-1;
						guiltSel=(my-350)/30;
					//si le joueur fait des hypotheses sur le suspect
					}else if ((mx>=250) && (mx<300) && (my>=350) && (my<740)) {
						int ind=(my-350)/30;
						guiltGuess[ind]=1-guiltGuess[ind];
					//si le joueur clique sur le bouton go
					}else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1)) {
						printf("go! joueur=%d objet=%d guilt=%d\n",joueurSel, objetSel, guiltSel);
						if (guiltSel!=-1) {
							sprintf(sendBuffer,"G %d %d",gId, guiltSel);
							//on envoi le message au serveur (on indique le joueur qu'on pense etre le coupable)
							sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
						}else if ((objetSel != -1) && (joueurSel == -1)) {
							sprintf(sendBuffer,"O %d %d",gId, objetSel);
							//on envoi le message au serveur (on pose une question a tout le monde a propos d'un objet) 
							sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
						}else if ((objetSel != -1) && (joueurSel != -1)) {
							sprintf(sendBuffer,"S %d %d %d",gId, joueurSel, objetSel);
							//on envoi le message au serveur (on pose une question a un seul joueur pour savoir combien de fois il possede l'objet)
							sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
						}
					}else {
						joueurSel=-1;
					...
					break;
				case SDL_MOUSEMOTION: SDL_GetMouseState( &mx, &my ); break;
				}
		}

        if (synchro==1) {
			printf("consomme |%s|\n",gbuffer);
			//on decode le message recu du serveur
			switch (gbuffer[0]) {
				// Message 'I' : le joueur recoit son Id
				case 'I':
					//On extrait l'id du joueur qui nous a ete attribue par le serveur
					sscanf(gbuffer,"I %d", &gId);
					printf("gId=%d\n",gId);
					break;
				// Message 'L' : le joueur recoit la liste des joueurs
				case 'L':
					//On extrait le nom des joueurs connectés sur la partie
				...
				case 'D':
					//On extrait les trois cartes du joueur
				...
				case 'M':
					//On extrait le numero du joueur courant
					int joueurCourant;
					sscanf(gbuffer, "M %d", &joueurCourant);
					printf("joueurCourant=%d\n",joueurCourant);
					if (joueurCourant == gId) // si c'est notre tour, on active le bouton go
						goEnabled = 1;
					else
						goEnabled = 0; //sinon, le bouton n'apparait pas
					break;
				// Message 'V' : le joueur recoit une valeur de tableCartes
				case 'V':
					//on extrait le numero du joueur, l'objet et le nombre de fois qu'il a cet objet
					int joueur, objet, valeur;
					sscanf(gbuffer, "V %d %d %d", &joueur, &objet, &valeur);
					printf("joueur=%d objet=%d valeur=%d\n",joueur, objet, valeur);
					if (joueur >= 0 && joueur < 4 && objet >= 0 && objet < 8) {
						tableCartes[joueur][objet] = valeur;
					}					
					break;
				case 'W':
					int id_vainqueur;
					sscanf(gbuffer, "W %d", &id_vainqueur);
					printf("Le joueur %s a gagne la partie !\n", gNames[id_vainqueur]);
					break;
				case 'E':
					int id_elimine;
					sscanf(gbuffer, "E %d", &id_elimine);
